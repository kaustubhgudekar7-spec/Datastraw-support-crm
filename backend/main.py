...
def signup(payload: schemas.AgentSignup, db: Session = Depends(get_db)):
    if crud.get_agent_by_email(db, payload.email):
        raise HTTPException(status_code=400, detail="An account with this email already exists")
    hashed = auth.hash_password(payload.password)
    agent = crud.create_agent(db, payload.name, payload.email, hashed)
    token = auth.create_access_token(agent.id, agent.email)
    return {"access_token": token, "agent": agent}


@app.post("/api/auth/login", response_model=schemas.TokenResponse)
def login(payload: schemas.AgentLogin, db: Session = Depends(get_db)):
    agent = crud.get_agent_by_email(db, payload.email)
...
@app.get("/api/auth/me", response_model=schemas.AgentOut)
def me(current_agent: models.Agent = Depends(auth.get_current_agent)):
    return current_agent


# ---------- API (ticket routes -- all require a logged-in agent) ----------

@app.post("/api/tickets", response_model=schemas.TicketCreateResponse)
def create_ticket(
    payload: schemas.TicketCreate,
    db: Session = Depends(get_db),
    current_agent: models.Agent = Depends(auth.get_current_agent),
):
